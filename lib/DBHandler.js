...
class DBHandler {
	...
	/**
	 * This function is used for the cron that checks and updates transaction entries in the database.
	 *
	 * @param {array} documents	Array of transaction documents that have been updated.
	 */
	static async updateTransactions(documents) {
		let promises = [];
		for (let i = 0; i < documents.length; i += 1) {
			promises.push(DBHandler.updateDocument('Transaction', { uuid: documents[i].uuid }, documents[i]));
		}
		let responses = await Promise.all(promises);

		// Check to make sure entries were updated, otherwise console log
		responses.forEach(response => {
			if (!response.pass) {
				console.log(response.data);
			} else {
				console.log('Successfully updated ' + response.data.uuid);
			}
		});
	}
